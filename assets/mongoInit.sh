...
if [ "$MJSM_OPLOG_SIZE" == "" ]; then
  ...
  MJSM_OPLOG_SIZE=$(($availDiskSpace*5/100))
fi
...
if [ "$MJSM_JOURNALING" == "false" ]; then
  mongod --port ${MJSM_MONGO_PORT} --replSet ${MJSM_REPLICA_SET_NAME} --oplogSize ${MJSM_OPLOG_SIZE} --directoryperdb --storageEngine $MJSM_DATABASE_ENGINE --nojournal &
else
  mongod --port ${MJSM_MONGO_PORT} --replSet ${MJSM_REPLICA_SET_NAME} --oplogSize ${MJSM_OPLOG_SIZE} --directoryperdb --storageEngine $MJSM_DATABASE_ENGINE &
fi
...
#  check for existing MJSM_MONGODB_PW env var (user-set) or generate
PW=${MJSM_MONGODB_PW:-$(pwgen -n -s -B 32 1)}
PW_TYPE=$( [ ${MJSM_MONGODB_PW} ] && echo "user-set" || echo "generated" )
echo ""
echo "Setting up MongoDB: "
echo "  - creation of the replica set configuration..."
mongo admin --eval "rs.initiate({_id: '${MJSM_REPLICA_SET_NAME}', members: [{_id: 0, host:'127.0.0.1:27017'}]})"
